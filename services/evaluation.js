...
const { OPERAND_TYPE, ARITHEMATIC_OPERATORS, RELATIONAL_OPERATORS, ZERO } = require("../constants/constants");
...
function evaluateRule(rule, userMarker, default_value = ZERO) {
    ...
    switch(rule.type) {
        case OPERAND_TYPE.BIO_MARKER: {
            ...
            console.warn(`userMarker ${rule.value} not found for ${userMarker.userId} using default Value ${default_value}`);
            return default_value;
        }
        ...
        case OPERAND_TYPE.ARITHEMATIC: {
            let left, right  = default_value, default_value;
            if (rule.left) {
            ...
            return evaluateArithematicOperation(left, right, rule.operator, default_value);
        }
        case OPERAND_TYPE.RELATIONAL_OPERATORS: {
            let left, right  = default_value, default_value;
            if (rule.left) {
                left = evaluateRule(rule.left, userMarker, default_value);
            }
